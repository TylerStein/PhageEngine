...
void SampleNormalMap(out vec3 outNormalValue){
	outNormalValue = texture(fNormalMap, fTexCoord).rgb * 2.0 - 1.0;
}

void SampleSpecularMap(out float outSpecValue){
	outSpecValue = texture(fSpecularMap, fTexCoord).r;
}
...
void Specular_Phong(int idx, vec3 lightDir, vec3 surfaceNormal, out float specularValue){
	...
	if(specularFactor < 0.0){
		...
		SampleSpecularMap(textureSpecular); //Sample the specular map (if one exists)
...
//TODO: Faster blinn-phong approximation reflection model
...
void Light(int idx, vec3 surfaceNormal, out vec3 diffuseOut, out vec3 specularOut, out vec3 ambientOut){
	...
	if(type == 0){
		//Point light
		vec3 lightVector = lPosition[idx] - fPos_world; //Get world space vector from light to surface
		lightDir = normalize(lightVector); //Create a normalized direction from the light-to-surface vector
		lambert = max(dot(lightDir, normalize(surfaceNormal)), 0.0); //Get the dot product of the surface normal and incoming light vector
		ExponentFalloff(lRange[idx], length(lightVector), attenuation); //Calculate the attenuation falloff for this light
	}else if(type == 1){
		//Spot light
		vec3 lightVector = lPosition[idx] - fPos_world; //Get world space vector from light to surface
		lightDir = normalize(lightVector); //Create a normalized direction from the light-to-surface vector
		...
		if(toSurface > (lCone[idx] + 1.0)){
			...
			lambert = max(dot(lightDir, normalize(surfaceNormal)), 0.0); //Get the dot product of the surface normal and incoming light vector
			ExponentFalloff(lRange[idx], length(lightVector), attenuation);
		}
		lambert = clamp(lambert * edgeFade, 0.0, lambert);
	}else if(type == 2){
		//Directional/infinite light
		vec3 lightVector = -lAngle[idx]; //Get world space vector from light to surface
		lightDir = normalize(lightVector); //Create a normalized direction from the light-to-surface vector
		lambert = max(dot(lightDir, normalize(surfaceNormal)), 0.0); //Get the dot product of the surface normal and incoming light vector
	...
	if(fShininess > 0.0){
		...
		Specular_Phong(idx, -lightDir, surfaceNormal, specularFactor);
...
void main(){
	...
	vec3 adjustedNormal = normalize(fTangentMatrix * normalMap);
	//vec3 adjustedNormal = normalize((fTangent_world * normalMap.x) + (fBiTangent_world * normalMap.y) + (fNormal_world * normalMap.z));
